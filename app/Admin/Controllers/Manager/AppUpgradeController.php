...
class AppUpgradeController extends AdminController
{
    ...
    public function form()
    {
        ...
        $form->select('channel_id', '渠道')
            ->options(AppUpgradeChannel::where('app_key', $app_key)->pluck('channel_name', 'id'))
            ->required()
        ...
        $form->saving(function (Form $form) use ($channel_id, $app_key) {
            if($form->isCreating()) {
            ...
            if($form->enabled == 'on' || $form->enabled == 1) {
                AppUpgrade::where('app_key', $app_key)
