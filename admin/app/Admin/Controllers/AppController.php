...
use App\Admin\Extensions\Widget\StatsInfoBox;
...
class AppController extends Controller
{
    ...
    public function list()
    {
        ...
        $grid->column('user_today', '今日新增注册')->display(function () use ($controller) {
            /** @var \App\Models\App $this */
            $stats = $controller->getAppStatsData($this->app_key);
    ...
    {
        $cache_key = "app_stats_data";
        
        return Cache::remember($cache_key, 60, function (){
            return $this->calculateStatsData();
        });
    }
     ...
     * 计算统计数据
     */
    protected function calculateStatsData()
    {
        // 获取当前租户的所有应用
        $apps = \App\Models\App::get(['app_key', 'platform_type']);
        ...
        $users_today = User::whereIn('app_key', $all_app_keys)
            ->whereBetween('created_at', [$today_start, $today_end])
            ->count();

        $users_yesterday = User::whereIn('app_key', $all_app_keys)
            ->whereBetween('created_at', [$yesterday_start, $yesterday_end])
            ->count();

        $users_total = User::whereIn('app_key', $all_app_keys)
            ->count();
        ...
        if (!empty($android_apps)) {
            $android_orders_today = Order::whereIn('app_key', $android_apps)
                ->whereBetween('created_at', [$today_start, $today_end])
                ->count();

            $android_orders_yesterday = Order::whereIn('app_key', $android_apps)
                ->whereBetween('created_at', [$yesterday_start, $yesterday_end])
                ->count();

            $android_orders_total = Order::whereIn('app_key', $android_apps)
                ->count();

            // Android使用payment_amount字段，status字段
            $android_income_today = Order::whereIn('app_key', $android_apps)
                ->where('status', 2)
                ->whereBetween('created_at', [$today_start, $today_end])
                ->sum('payment_amount');

            $android_income_yesterday = Order::whereIn('app_key', $android_apps)
                ->where('status', 2)
                ->whereBetween('created_at', [$yesterday_start, $yesterday_end])
                ->sum('payment_amount');

            $android_income_total = Order::whereIn('app_key', $android_apps)
                ->where('status', 2)
                ->sum('payment_amount');
        }
        ...
        if (!empty($ios_apps)) {
            // iOS订单统计 - 排除沙盒环境数据
            $ios_orders_today = AppleOrder::whereIn('app_key', $ios_apps)
                ->where('environment', AppleOrder::ENVIRONMENT_PRODUCTION)
                ->whereBetween('created_at', [$today_start, $today_end])
                ->count();

            $ios_orders_yesterday = AppleOrder::whereIn('app_key', $ios_apps)
                ->where('environment', AppleOrder::ENVIRONMENT_PRODUCTION)
                ->whereBetween('created_at', [$yesterday_start, $yesterday_end])
                ->count();

            $ios_orders_total = AppleOrder::whereIn('app_key', $ios_apps)
                ->where('environment', AppleOrder::ENVIRONMENT_PRODUCTION)
                ->count();

            // iOS收入统计 - 排除沙盒环境数据，使用amount字段，payment_status字段
            $ios_income_today = AppleOrder::whereIn('app_key', $ios_apps)
                ->where('environment', AppleOrder::ENVIRONMENT_PRODUCTION)
                ->where('payment_status', AppleOrder::PAYMENT_STATUS_SUCCESS)
                ->whereBetween('created_at', [$today_start, $today_end])
                ->sum('amount');

            $ios_income_yesterday = AppleOrder::whereIn('app_key', $ios_apps)
                ->where('environment', AppleOrder::ENVIRONMENT_PRODUCTION)
                ->where('payment_status', AppleOrder::PAYMENT_STATUS_SUCCESS)
                ->whereBetween('created_at', [$yesterday_start, $yesterday_end])
                ->sum('amount');

            $ios_income_total = AppleOrder::whereIn('app_key', $ios_apps)
                ->where('environment', AppleOrder::ENVIRONMENT_PRODUCTION)
                ->where('payment_status', AppleOrder::PAYMENT_STATUS_SUCCESS)
                ->sum('amount');
        }
    ...
    {   
        ...
        // 清除所有应用的缓存
        $app_keys = App::pluck('app_key')->toArray();
        foreach ($app_keys as $app_key) {
            $app_cache_key = "single_app_stats_data_app_{$app_key}";
     ...
     * 缓存时间：60秒
     */
    public function getAppStatsData($app_key)
    {
        $cache_key = "single_app_stats_data_app_{$app_key}";
        
        return Cache::remember($cache_key, 60, function () use ($app_key) {
            return $this->calculateSingleAppStatsData($app_key);
        });
    }
     ...
     * 计算单个应用的统计数据
     */
    protected function calculateSingleAppStatsData($app_key)
    {
        // 获取应用平台类型
        $app = App::where('app_key', $app_key)
            ->first(['platform_type']);
        ...
        // 用户统计（所有平台通用）
        $users_today = User::where('app_key', $app_key)
            ->whereBetween('created_at', [$today_start, $today_end])
            ->count();

        $users_yesterday = User::where('app_key', $app_key)
            ->whereBetween('created_at', [$yesterday_start, $yesterday_end])
            ->count();

        $users_total = User::where('app_key', $app_key)
            ->count();
        ...
        // 订单统计 - iOS平台排除沙盒环境数据
        $order_query_base = $order_model::where('app_key', $app_key);
        if ($is_ios) {
        ...
        $income_query_base = $order_model::where('app_key', $app_key)
