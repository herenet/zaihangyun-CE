...
use App\Admin\Actions\AppConfigCopy;
...
class AppConfigController extends AdminController
{
    ...
    public function grid()
    {
        ...
        $grid->model()->where('app_key', $app_key);
    ...
    public function form()
    {
        ...
        $form->text('name', '配置标识符')
            ->required()
            ->rules(function($form) use ($app_key) {
                $id = $form->model()->id;
                return [
                    'required',
                    'string',
                    'max:32',
                    'regex:/^[a-z][a-z0-9_]*$/',
                    Rule::unique('app_configs')->where(function ($query) use ($app_key) {
