...
class IndexController extends AdminController
{
    ...
    protected function moduleList()
    {
        ...
        $order_interface_config = app(OrderInterfaceConfig::class)->getConfig($app_key);
        $user_interface_config = app(LoginInterfaceConfig::class)->getConfig($app_key);
        $article_config = app(ArticleConfig::class)->getConfig($app_key);
        $message_config = app(MessageConfig::class)->getConfig($app_key);
        // 模块列表及其描述
        $modules = [
            [
                'name' => '应用设置',
            ...
            [
                'name' => '售卖管理',
                'description' => '提供商品、订单及收退款管理，兼容微信、支付宝、Apple IAP等支付通道，支持API调用。',
    ...
    protected function stats()
    {
        ...
        // 用户统计
        $user_count = User::where('app_key', $app_key)
            ->count();
        $user_increate = User::where('app_key', $app_key)
            ->whereBetween('created_at', [$today_start, $today_end])
            ->count();
        $user_yesterday = User::where('app_key', $app_key)
