                ...
                $form->radioButton('subscrip_switch', '订阅功能')
                    ...
                    ->when(1, function (Form $form) {
                        ...
                        $form->select('apple_dev_s2s_config_id', '苹果服务端API证书')
                            ...
                            ->options(function () {
                                return AppleDevS2SConfig::where(['interface_check' => 1])->get()->pluck('dev_account_name', 'id');
                            })
                        ...
                        $form->display('apple_pay_callback_url_prd', '苹果IAP回调地址')
                            ->readonly()
                            ->with(function ($value) {
                                $params = Helpers::simpleEncode($this->getAppKey());
...
JS);
    ...
    public function data()
    {
        ...
        $iap_config = app(IAPConfigModel::class)->getConfig($this->getAppKey());
        return[
